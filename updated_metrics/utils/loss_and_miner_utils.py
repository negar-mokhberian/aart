...
def get_matches_and_diffs(labels, ref_labels=None, mask_labels=None):
    ...
    mask_mat = (mask_labels.unsqueeze(1) == mask_labels.unsqueeze(0)).byte()
    matches = matches * mask_mat
    diffs = diffs * mask_mat
    return matches, diffs


def get_all_pairs_indices(labels, ref_labels=None, mask_labels=None):
    """
    Given a tensor of labels, this will return 4 tensors.
    The first 2 tensors are the indices which form all positive pairs
    The second 2 tensors are the indices which form all negative pairs
    """
    matches, diffs = get_matches_and_diffs(labels, ref_labels, mask_labels=mask_labels)
    a1_idx, p_idx = torch.where(matches)
    a2_idx, n_idx = torch.where(diffs)
    return a1_idx, p_idx, a2_idx, n_idx


def convert_to_pairs(indices_tuple, labels, ref_labels=None, mask_labels=None):
    """
    ...
    if indices_tuple is None:
        return get_all_pairs_indices(labels, ref_labels, mask_labels=mask_labels)
